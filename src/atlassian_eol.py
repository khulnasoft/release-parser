...
from common import dates, http, releaselog, releaserlog
...
such as `JiraSoftware` (from `AtlassianSupportReleaselogPolicy-JiraSoftware`).
"""
...
for config in releaselog.list_configs(p_filter, METHOD, m_filter):
    with releaserlog.ProductData(config.product) as product_data:
        ...
        for li in soup.select(f"#AtlassianSupportReleaselogPolicy-{config.url}+ul li"):
