...
from common import dates, releaserlog
from common.git import Git

"""Fetch Debian versions by parsing news in www.debian.org source repository."""


def extract_major_versions(p: releaserlog.ProductData, repo_dir: Path) -> None:
    child = run(
...
def extract_point_versions(p: releaserlog.ProductData, repo_dir: Path) -> None:
    child = run(
...
with releaserlog.ProductData("debian") as product_data:
